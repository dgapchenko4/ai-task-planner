    ...
    def cte(
        ...
        Example 1, non recursive::
            ...
            statement = select(
                        orders.c.region,
                        orders.c.book,
                        func.sum(orders.c.quantity).label("book_units"),
                        func.sum(orders.c.amount).label("book_sales")
                ).where(orders.c.region.in_(
                    select(top_regions.c.region)
                )).group_by(orders.c.region, orders.c.book)
        ...
        Example 3, an upsert using UPDATE and INSERT with CTEs::
            ...
            # add 5 visitors for the book_id == 1
            book_id = 1
            day = date.today()
            count = 5
            ...
            update_cte = (
                visitors.update()
                .where(and_(visitors.c.book_id == book_id,
                            visitors.c.date == day))
            ...
            upsert = visitors.insert().from_select(
                [visitors.c.book_id, visitors.c.date, visitors.c.count],
                select(literal(book_id), literal(day), literal(count))
