...
class Linting(IntEnum):
    ...
    this feature currently includes support for flagging cartesian books
    in SQL statements.
    ...
    COLLECT_CARTESIAN_BOOKS = 1
    """Collect data on FROMs and cartesian books and gather into
    'self.from_linter'"""

    WARN_LINTING = 2
    "Emit warnings for linters that find problems"

    FROM_LINTING = COLLECT_CARTESIAN_BOOKS | WARN_LINTING
    """Warn for cartesian books; combines COLLECT_CARTESIAN_BOOKS
    and WARN_LINTING"""


NO_LINTING, COLLECT_CARTESIAN_BOOKS, WARN_LINTING, FROM_LINTING = tuple(
    Linting
)


class FromLinter(collections.namedtuple("FromLinter", ["froms", "edges"])):
    """represents current state for the "cartesian book" detection
    feature."""
    ...
    def warn(self, stmt_type="SELECT"):
        ...
        if the_rest:
            ...
            if froms:
                template = (
                    "{stmt_type} statement has a cartesian book between "
...
class SQLCompiler(Compiled):
    ...
    **kw)`` and returns a select object.   this is used for structural changes
    ...
    def _compose_select_body(
        ...
        if self.linting & COLLECT_CARTESIAN_BOOKS:
